...
void arrow_switch_view(const Arg* arg);
...
#define LASTTAG (1 << (LENGTH(tags) - 1))
void arrow_switch_view(const Arg* arg) {
	int current_tag = selmon->tagset[selmon->seltags];
	if(arg->ui) // go forwards
		if(!(current_tag & LASTTAG)) current_tag <<= 1; else return;
	else // go backwards
		if(current_tag != 1) current_tag >>= 1; else return;

	Arg a = {.ui = current_tag};
	view(&a);
}
